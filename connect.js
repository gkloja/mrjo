...
const corsOptions = {
    methods: ['GET', 'POST'], // Remove PUT, DELETE se não usar
    allowedHeaders: [
        'Content-Type', 
        'Authorization', 
        'x-user-id', 
...
// Limite mais restrito para consultas
const consultaLimiter = rateLimit({
    windowMs: 60 * 60 * 1000, // 1 hora
    max: 100, // máximo 30 consultas por hora
    message: {
        success: false,
...
// Aplicar limites
app.use('/api', limiter);
app.use('/api/consultar', consultaLimiter);

// Middleware para validar headers do frontend
/*
//connectMongoDb();
...
// ==========================================
// MIDDLEWARE DE DEBUG - Adicione ANTES das rotas
// ==========================================
// ==========================================
// MIDDLEWARE PARA ACEITAR TEXT/PLAIN COMO JSON
// ==========================================
app.use((req, res, next) => {
    // Se for POST e o Content-Type for text/plain, tenta parsear como JSON
    if (req.method === 'POST' && req.headers['content-type']?.includes('text/plain')) {
        ...
        req.on('end', () => {
            try {
                ...
                console.log('✅ Convertido text/plain para JSON:', req.body);
            } catch(e) {
                console.log('⚠️ Não foi possível converter para JSON:', body);
                req.body = {};
            }
            next();
        });
...
// Depois use o express.json normalmente
app.use(express.json({ limit: '10mb' }));
app.use(express.urlencoded({ extended: true }));

// Se quiser capturar o raw body também (útil para debug)
...
const LOGS_FILE = path.join(__dirname, 'public', 'logs_consultas.json');
...
// ==========================================
// FUNÇÕES DE LEITURA/ESCRITA (CORRETAMENTE POSICIONADAS)
// ==========================================
function lerCreditos() {
    try {
      
        return JSON.parse(fs.readFileSync(CREDITOS_FILE, 'utf8'));
    } catch (error) {
...
function lerIPs() {
    try {
        ...
        console.error('Erro ao ler IPs:', error);
        return {};
    }
}
...
function salvarCreditos(dados) {
    try {
        const caminho = CREDITOS_FILE;
        const conteudo = JSON.stringify(dados, null, 2);
        
        console.log(`💾 Salvando créditos em: ${caminho}`);
        console.log(`   Tamanho: ${conteudo.length} bytes`);
        
        fs.writeFileSync(caminho, conteudo, 'utf8');
        
        // Verifica se escreveu corretamente
        const verificacao = fs.readFileSync(caminho, 'utf8');
        const dadosVerificados = JSON.parse(verificacao);
        
        if (Object.keys(dadosVerificados).length === Object.keys(dados).length) {
            console.log(`✅ Créditos salvos e verificados!`);
            return true;
        } else {
...
function salvarIPs(dados) {
    try {
        const caminho = IPS_FILE;
        const conteudo = JSON.stringify(dados, null, 2);
        
        console.log(`💾 Salvando IPs em: ${caminho}`);
        console.log(`   Tamanho: ${conteudo.length} bytes`);
        ...
        if (Object.keys(dadosVerificados).length === Object.keys(dados).length) {
            console.log(`✅ IPs salvos e verificados!`);
            return true;
        } else {
            console.log(`⚠️ Verificação falhou!`);
            return false;
        ...
        console.error('❌ Erro ao salvar IPs:', error);
        return false;
    }
}

function salvarLogs(dados) {
    try {
        fs.writeFileSync(LOGS_FILE, JSON.stringify(dados, null, 2));
...
// ==========================================
// ==========================================
// API: SALVAR CRÉDITOS (JÁ CORRETA)
// ==========================================

// ==========================================
// SISTEMA DE AFILIADOS - BACKEND
// ==========================================

// Gerar código único para afiliado
function gerarCodigoAfiliado(userId) {
    const parte1 = userId.substring(0, 6);
    const parte2 = Date.now().toString(36).toUpperCase().slice(-6);
    return `${parte1}${parte2}`.toUpperCase();
}

// Registrar afiliado
// ==========================================
// API: REGISTRAR AFILIADO (GERAR CÓDIGO)
// ==========================================
app.post('/api/registrar-afiliado', (req, res) => {
    ...
    try {
        ...
        if (dados[userId].afiliado && dados[userId].afiliado.codigo) {
            return res.json({ 
                success: true, 
                codigo: dados[userId].afiliado.codigo,
                jaExistente: true 
            });
        }
        ...
        function gerarCodigo() {
            const parte1 = userId.substring(0, 6).toUpperCase();
            const parte2 = Date.now().toString(36).slice(-6).toUpperCase();
        ...
        do {
            codigo = gerarCodigo();
            existe = Object.values(dados).some(u => u.afiliado?.codigo === codigo);
        } while (existe);
        
        // Criar estrutura de afiliado
        dados[userId].afiliado = {
            codigo: codigo,
            totalIndicacoes: 0,
            creditosGanhos: 0,
            indicacoes: [],
        ...
        salvarCreditos(dados);
        
        console.log(`✅ Afiliado registrado: ${nome || userId} - Código: ${codigo}`);
        
        res.json({ 
            success: true, 
            codigo: codigo,
            link: `${req.protocol}://${req.get('host')}?ref=${codigo}`,
            novo: true 
        });
...
// Processar indicação
app.post('/api/processar-indicacao', (req, res) => {
    const { codigoAfiliado, novoUserId, novoNome } = req.body;
    
    if (!codigoAfiliado || !novoUserId) {
    ...
    try {
        ...
        // Procurar afiliado pelo código
        let afiliadoId = null;
        let afiliado = null;
        
        for (const [id, user] of Object.entries(dados)) {
            if (user.afiliado && user.afiliado.codigo === codigoAfiliado) {
                afiliadoId = id;
                afiliado = user;
                break;
            }
        ...
        if (!afiliadoId) {
            return res.status(404).json({ success: false, error: 'Código de afiliado inválido' });
        }
        
        // Verifica se não é o próprio usuário
        if (afiliadoId === novoUserId) {
            return res.status(403).json({ success: false, error: 'Não pode indicar a si mesmo' });
        }
        
        // Verifica se já foi indicado por este afiliado
        if (afiliado.afiliado.indicacoes.includes(novoUserId)) {
            return res.json({ success: false, error: 'Usuário já foi indicado', jaIndicado: true });
        }
        
        // Adicionar indicação
        dados[afiliadoId].afiliado.indicacoes.push(novoUserId);
        dados[afiliadoId].afiliado.totalIndicacoes += 1;
        
        // Ganha 1 crédito
        const creditosGanhos = 1;
        dados[afiliadoId].afiliado.creditosGanhos += creditosGanhos;
        dados[afiliadoId].creditos = (dados[afiliadoId].creditos || 5) + creditosGanhos;
        
        // Marcar no novo usuário quem o indicou
        if (!dados[novoUserId].indicadoPor) {
            dados[novoUserId].indicadoPor = {
        ...
        console.log(`✅ ${afiliado.nome} ganhou 1 crédito por indicar ${novoNome || novoUserId}`);
        console.log(`📊 Total de indicações: ${dados[afiliadoId].afiliado.totalIndicacoes}`);
        ...
        res.json({ 
            ...
            afiliado: {
                id: afiliadoId,
                nome: afiliado.nome,
                totalIndicacoes: dados[afiliadoId].afiliado.totalIndicacoes,
                creditosGanhos: dados[afiliadoId].afiliado.creditosGanhos
            }
        });
...
app.get('/api/buscar-afiliado', (req, res) => {
    ...
    try {
        ...
        for (const [id, user] of Object.entries(dados)) {
            if (user.afiliado && user.afiliado.codigo === codigo) {
                return res.json({ 
                    success: true, 
                    afiliado: {
                        id: id,
                        nome: user.nome,
                        codigo: user.afiliado.codigo
                    }
                });
            }
        }
        
        res.json({ success: false, error: 'Código não encontrado' });
...
app.get('/api/estatisticas-afiliado', (req, res) => {
    ...
    try {
        ...
        if (!user || !user.afiliado) {
            return res.json({ 
                success: true, 
                temAfiliado: false,
                mensagem: 'Você ainda não é um afiliado. Compartilhe seu link para começar!'
            });
        }
        ...
        res.json({ 
            ...
            afiliado: {
                codigo: user.afiliado.codigo,
                totalIndicacoes: user.afiliado.totalIndicacoes,
                creditosGanhos: user.afiliado.creditosGanhos,
                dataCriacao: user.afiliado.dataCriacao,
...
app.post('/api/salvar-creditos', (req, res) => {
    ...
    console.log(`📝 Salvando créditos - User: ${userId}, Créditos: ${creditos}, IP: ${ip}`);
    ...
    if (creditos === undefined || creditos === null) {
        return res.status(400).json({ success: false, error: 'creditos obrigatório' });
    }
    ...
    try {
        ...
        // ✅ VERIFICAÇÃO CORRETA: Permite se for o MESMO usuário
        if (dadosIPs[ip] && dadosIPs[ip].userId !== userId) {
            console.log(`❌ Bloqueado - IP ${ip} pertence a ${dadosIPs[ip].userId}, tentativa com ${userId}`);
            return res.status(403).json({ 
                success: false, 
                error: 'Não pode criar diversas contas. Entre em contato: +55 21 98904-7220'
            });
        }
        
        // Salva ou atualiza créditos
        dadosCreditos[userId] = {
            creditos: parseInt(creditos),
            ultimaAtualizacao: new Date().toISOString(),
            ip: ip,
            nome: nome || dadosCreditos[userId]?.nome || 'Marujo',
            email: email || dadosCreditos[userId]?.email || ''
        };
        
        // Registra ou atualiza IP
        if (!dadosIPs[ip]) {
            dadosIPs[ip] = { 
                userId: userId, 
                dataCriacao: new Date().toISOString(),
            ...
            dadosIPs[ip].ultimoAcesso = new Date().toISOString();
            // Garante que o userId está correto (mesmo usuário)
            dadosIPs[ip].userId = userId;
        }
        
        salvarCreditos(dadosCreditos);
        salvarIPs(dadosIPs);
        
        console.log(`✅ Créditos salvos: ${userId} = ${creditos}`);
        res.json({ success: true, creditos: parseInt(creditos) });
...
// ==========================================
// API: CARREGAR CRÉDITOS (JÁ CORRETA)
// ==========================================
app.get('/api/carregar-creditos', (req, res) => {
    const { userId } = req.query;
    const ip = getClientIp(req);
    
    console.log(`📥 Carregando créditos - User: ${userId}, IP: ${ip}`);
    ...
    try {
        ...
        // ✅ VERIFICAÇÃO CORRETA: Permite se for o MESMO usuário
        if (dadosIPs[ip] && dadosIPs[ip].userId !== userId) {
            console.log(`❌ IP conflitante - IP ${ip} pertence a ${dadosIPs[ip].userId}`);
            return res.status(403).json({ 
                success: false, 
                error: 'Não pode criar diversas contas. Entre em contato: +55 21 98904-7220',
                bloqueado: true
        ...
        const creditos = usuario?.creditos !== undefined ? usuario.creditos : 5;
        
        console.log(`✅ Créditos carregados: ${userId} = ${creditos}`);
        
        // Se o IP ainda não está registrado, registra agora
        if (!dadosIPs[ip]) {
            dadosIPs[ip] = {
                userId: userId,
                dataCriacao: new Date().toISOString(),
            ...
            salvarIPs(dadosIPs);
            console.log(`🆕 IP ${ip} registrado para ${userId}`);
        }
        
        res.json({ success: true, creditos: creditos });
...
// ==========================================
// ==========================================
// API: REGISTRAR USUÁRIO (COM DEBUG COMPLETO)
// ==========================================
// NO FRONTEND - substitua as funções para usar o backend

// Gerar link de afiliado (usando backend)
async function gerarLinkAfiliado() {
    if (!currentUser) {
        showToast('🔒 Faça login para gerar seu link de afiliado!', 'error');
        return;
    }
    
    try {
        const response = await fetch(`${SERVER_API}/api/registrar-afiliado`, {
            method: 'POST',
        ...
        if (data.success) {
            const link = `${window.location.origin}${window.location.pathname}?ref=${data.codigo}`;
            navigator.clipboard.writeText(link);
            showToast(`✅ Link copiado! Compartilhe e ganhe 1 crédito por cada amigo!`, 'success');
        } else {
...
async function mostrarEstatisticasAfiliado() {
    if (!currentUser) {
        showToast('🔒 Faça login para ver suas estatísticas!', 'error');
        return;
    }
    
    try {
        const response = await fetch(`${SERVER_API}/api/estatisticas-afiliado?userId=${currentUser.uid}`);
        const data = await response.json();
        
        if (data.success && data.temAfiliado) {
            const msg = `
📊 *MINHAS ESTATÍSTICAS DE AFILIADO*

🔗 Código: ${data.afiliado.codigo}
👥 Indicações: ${data.afiliado.totalIndicacoes}
💰 Créditos ganhos: ${data.afiliado.creditosGanhos}
🎁 Créditos atuais: ${data.afiliado.creditosAtuais}

💡 Cada amigo que se cadastrar pelo seu link,
você ganha 1 CRÉDITO GRÁTIS!
            `;
            
            const link = `${window.location.origin}${window.location.pathname}?ref=${data.afiliado.codigo}`;
            const whatsappLink = `https://wa.me/?text=${encodeURIComponent(msg + '\n\nLink: ' + link)}`;
            window.open(whatsappLink, '_blank');
        } else {
            showToast(data.mensagem || 'Você ainda não é afiliado. Compartilhe seu link!', 'info');
            // Se não é afiliado, gera o link automaticamente
            gerarLinkAfiliado();
        }
    } catch (error) {
        console.error('Erro:', error);
...
function verificarCodigoAfiliado() {
    const urlParams = new URLSearchParams(window.location.search);
    const codigo = urlParams.get('ref');
    
    if (codigo) {
        console.log(`🔗 Link com código de afiliado: ${codigo}`);
        sessionStorage.setItem('codigo_afiliado_pendente', codigo);
    }
}
app.post('/api/registrar-usuario', (req, res) => {
    const ip = getClientIp(req);
    
    // Extrai dados de qualquer lugar
    let userId = null;
    let nome = null;
    let email = null;
    let creditosIniciais = 5;
    let codigoAfiliado = null; // NOVO: código do afiliado
    ...
    if (req.body) {
        if (typeof req.body === 'object') {
            ...
            nome = req.body.nome;
            email = req.body.email;
            creditosIniciais = req.body.creditosIniciais || 5;
            codigoAfiliado = req.body.codigoAfiliado || req.body.ref; // NOVO
            ...
            try {
                ...
                nome = parsed.nome;
                email = parsed.email;
                creditosIniciais = parsed.creditosIniciais || 5;
                codigoAfiliado = parsed.codigoAfiliado || parsed.ref; // NOVO
            } catch(e) {}
        }
    }
    ...
    // Tenta da query string (para links diretos) - NOVO
    if (!codigoAfiliado && req.query) {
        codigoAfiliado = req.query.ref || req.query.codigo;
    }
    ...
    console.log('📌 creditosIniciais:', creditosIniciais);
    console.log('📌 codigoAfiliado:', codigoAfiliado); // NOVO
    ...
    try {
        ...
        console.log('📁 Estado atual dos arquivos:');
        console.log('   Usuários cadastrados:', Object.keys(dadosCreditos).length);
        console.log('   IPs cadastrados:', Object.keys(dadosIPs).length);
        
        // ==========================================
        // VERIFICAÇÃO DE IP (DEVE BLOQUEAR)
        // ==========================================
        if (dadosIPs[ip]) {
            const donoDoIp = dadosIPs[ip].userId;
            console.log(`🔍 IP ${ip} já pertence a: ${donoDoIp}`);
            
            if (donoDoIp !== userId) {
                console.log(`❌ BLOQUEADO! Tentativa de criar conta diferente no mesmo IP`);
                return res.status(403).json({ 
                    success: false, 
                    error: 'Este IP já possui uma conta registrada. Não é permitido múltiplas contas.',
                    debug: {
                        ip: ip,
                        conta_existente: donoDoIp,
                        tentativa: userId
                    }
                });
            } else {
                console.log(`✅ Mesmo usuário, permitindo acesso`);
            }
        }
        ...
        if (dadosCreditos[userId]) {
            console.log(`✅ Usuário ${userId} já existe, retornando créditos`);
            ...
            const salvouIPs = salvarIPs(dadosIPs);
            console.log(`   Salvou IPs? ${salvouIPs ? '✅' : '❌'}`);
            
            return res.json({ 
                success: true, 
                jaExistente: true, 
                creditos: dadosCreditos[userId].creditos,
                debug: { ip, userId }
            });
        }
        ...
        const creditosNumero = parseInt(creditosIniciais) || 5;
        const agora = new Date().toISOString();
        
        // ==========================================
        // SISTEMA DE AFILIADOS - PROCESSAR INDICAÇÃO
        // ==========================================
        let indicacaoProcessada = null;
        
        if (codigoAfiliado) {
            console.log(`🔗 Código de afiliado detectado: ${codigoAfiliado}`);
            
            // Procurar o afiliado pelo código
            let afiliadoId = null;
            let afiliadoData = null;
            
            for (const [id, user] of Object.entries(dadosCreditos)) {
                if (user.afiliado && user.afiliado.codigo === codigoAfiliado) {
                    afiliadoId = id;
                    afiliadoData = user;
                    break;
            ...
            if (afiliadoId && afiliadoId !== userId) {
                // Verifica se o novo usuário já não foi indicado antes
                const jaIndicado = afiliadoData.afiliado.indicacoes?.includes(userId);
                
                if (!jaIndicado) {
                    // Adiciona indicação
                    if (!afiliadoData.afiliado.indicacoes) {
                        afiliadoData.afiliado.indicacoes = [];
                    }
                    afiliadoData.afiliado.indicacoes.push(userId);
                    afiliadoData.afiliado.totalIndicacoes = (afiliadoData.afiliado.totalIndicacoes || 0) + 1;
                    
                    // Adiciona crédito ao afiliado (1 crédito por indicação)
                    const creditosGanhos = 1;
                    afiliadoData.afiliado.creditosGanhos = (afiliadoData.afiliado.creditosGanhos || 0) + creditosGanhos;
                    afiliadoData.creditos = (afiliadoData.creditos || 5) + creditosGanhos;
                    
                    // Atualiza o arquivo
                    dadosCreditos[afiliadoId] = afiliadoData;
                    
                    indicacaoProcessada = {
                        afiliadoId: afiliadoId,
                        afiliadoNome: afiliadoData.nome,
                        creditosGanhos: creditosGanhos,
                        totalIndicacoes: afiliadoData.afiliado.totalIndicacoes
                    };
                    
                    console.log(`✅ INDICAÇÃO PROCESSADA: ${afiliadoData.nome} ganhou 1 crédito por indicar ${nome || userId}`);
                    console.log(`📊 Total de indicações do afiliado: ${afiliadoData.afiliado.totalIndicacoes}`);
                } else {
                    console.log(`⚠️ Usuário ${userId} já foi indicado por ${afiliadoData.nome}`);
                }
            } else if (afiliadoId === userId) {
                console.log(`⚠️ Usuário tentou usar seu próprio código de afiliado`);
            } else {
                console.log(`❌ Código de afiliado ${codigoAfiliado} não encontrado`);
            }
        }
        ...
        // Prepara dados (incluindo indicação)
        dadosCreditos[userId] = {
            ...
            ultimoAcesso: agora,
            ip: ip,
            // Guarda quem indicou o usuário
            indicadoPor: indicacaoProcessada ? {
                afiliadoId: indicacaoProcessada.afiliadoId,
                codigo: codigoAfiliado,
                data: agora
            } : null
        };
        ...
        // TENTA SALVAR OS ARQUIVOS
        console.log('📝 Tentando salvar arquivos...');
        
        const salvouCreditos = salvarCreditos(dadosCreditos);
        const salvouIPs = salvarIPs(dadosIPs);
        
        console.log(`   Salvou créditos? ${salvouCreditos ? '✅' : '❌'}`);
        console.log(`   Salvou IPs? ${salvouIPs ? '✅' : '❌'}`);
        
        // VERIFICA SE REALMENTE SALVOU
        const verificaCreditos = lerCreditos();
        const verificaIPs = lerIPs();
        
        const usuarioSalvo = verificaCreditos[userId];
        const ipSalvo = verificaIPs[ip];
        
        console.log(`   Verificação pós-salvamento:`);
        console.log(`     Usuário existe no JSON? ${usuarioSalvo ? '✅' : '❌'}`);
        console.log(`     IP existe no JSON? ${ipSalvo ? '✅' : '❌'}`);
        
        if (usuarioSalvo && ipSalvo) {
            console.log(`✅ Usuário CRIADO e SALVO com sucesso!`);
            
            // Resposta com informações da indicação
            const resposta = { 
                success: true, 
                novo: true, 
                creditos: creditosNumero,
                debug: { 
                    userId, 
                    ip,
                    arquivo_creditos: CREDITOS_FILE,
                    arquivo_ips: IPS_FILE
                }
            };
            
            // Se teve indicação, inclui na resposta
            if (indicacaoProcessada) {
                resposta.indicacao = {
                    processada: true,
                    afiliado: indicacaoProcessada.afiliadoNome,
                    creditosGanhos: indicacaoProcessada.creditosGanhos,
                    mensagem: `✅ Você foi indicado por ${indicacaoProcessada.afiliadoNome}!`
                };
            }
            
            res.json(resposta);
        } else {
            console.log(`❌ FALHA AO SALVAR! Dados não persistiram.`);
            throw new Error('Dados não foram salvos no arquivo');
        }
...
app.get('/api/verificar-ip', (req, res) => {
    ...
    try {
        ...
        if (dadosIPs[ip]) {
            console.log(`✅ IP ${ip} pertence a: ${dadosIPs[ip].userId}`);
            return res.json({
                success: true,
                temConta: true,
                userId: dadosIPs[ip].userId,
                dataCriacao: dadosIPs[ip].dataCriacao
            });
        }
        
        console.log(`🆕 IP ${ip} não possui conta`);
...
// ==========================================
// API DE CONSULTA (SUA EXISTENTE)
// ==========================================
// ... mantenha sua rota /api/consultar aqui ...

// ==========================================
// FUNÇÕES AUXILIARES DE CRÉDITOS (PARA A CONSULTA)
// ==========================================
...
switch(tipo) {
    ...
    case 'cnpj':
        ...
        break;
    case 'placa':
        // CORREÇÃO: Força usar v2 se não especificado
        const versaoPlaca = versao || 'v2';
        url = `${BASE_URL}placa/${versaoPlaca}?apikey=${API_KEY}&placa=${valor}`;
        console.log(`🔍 Consultando placa versão ${versaoPlaca}: ${url}`);
        break;
    case 'telefone':
        url = `${BASE_URL}telefone/v1?apikey=${API_KEY}&telefone=${valor}`;
        break;
    case 'motor':
        // CORREÇÃO: Para motor, a URL precisa ser construída corretamente
        // O valor já vem como o número do motor (ex: UD316909)
        url = `${BASE_URL}motor/v1?apikey=${API_KEY}&tipo=motor&valor=${valor}`;
        console.log(`🔍 Consultando motor: ${valor}`);
        break;
    case 'chassi':
        // CORREÇÃO: Para chassi
        url = `${BASE_URL}motor/v1?apikey=${API_KEY}&tipo=chassi&valor=${valor}`;
        console.log(`🔍 Consultando chassi: ${valor}`);
        break;
    case 'foto':
